...
- FRC 모드에서 현재 추가로 보는 값
  ...
  - 즉 현재 `FRC`는 `STRICT`를 베이스로 하되 “ERS 퍼센트 + DRS 활성화 여부”를 추가 노출
...
- `FRC` 모드는 현재 `STRICT`를 기본 베이스로 두되, 먼저 `ERS` 퍼센트와 `DRS` 활성화 여부를 추가 노출합니다. 추후 사용자가 지정할 FRC 전용 필드/규칙을 더 넣을 수 있게 설정 타입도 열어두었습니다.
